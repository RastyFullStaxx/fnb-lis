...
import { saleCreate, voidRequest } from "@fnb/core";
...
export const saleRoutes = new Hono<AppEnv>()
  ...
  /** Void + linked replacement in one step. */
  .post("/sales/:id/correct", voidGuard, zValidator("json", saleCreate.and(voidRequest)), async (c) => {
    const location = c.get("location");
    const user = c.get("user")!;
    const body = c.req.valid("json");
    const original = await prisma.saleRecord.findUnique({ where: { id: c.req.param("id") } });
    ...
    const replacement = await prisma.$transaction(async (tx) => {
      ...
      await logActivity(
        { user, clientId: location.clientId, locationId: location.id, action: "sale.correct", entity: "SaleRecord", entityId: created.id, summary: `Corrected record: ${body.reason}`, details: { originalId: original.id } },
        tx,
      );
      return created;
    });
