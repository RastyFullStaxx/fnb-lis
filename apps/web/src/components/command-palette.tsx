    ...
    <>
      {(items.data ?? []).length > 0 && (
        <CommandGroup heading="Items">
          {items.data!.slice(0, 200).map((li) => {
            ...
            return (
              <CommandItem key={li.id} value={`item ${label}`} onSelect={() => onGo("stock")}>
      ...
      {(menus.data ?? []).length > 0 && (
        <CommandGroup heading="Menus">
          {menus.data!.map((m) => (
            <CommandItem key={m.id} value={`menu ${m.name}`} onSelect={() => onGo("recipes")}>
      ...
      {(suppliers.data ?? []).length > 0 && (
        <CommandGroup heading="Suppliers">
          {suppliers.data!.map((s) => (
            <CommandItem key={s.id} value={`supplier ${s.name}`} onSelect={() => onGo("suppliers")}>
