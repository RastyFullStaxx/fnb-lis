 ...
 * Industry (client req 2026-07-24) is shown read-only here — it's a
 * category-level field (one industry per category, e.g. "Dental
 * Equipment" -> Dental), edited in Main Database's Category dialog, not
 * per physical asset.
 *
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Serial number, condition, status, initial cost, and remarks for this location's register. Industry is set on the item's category in Main Database.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <p className="flex h-9 items-center rounded-md border border-transparent px-3 text-sm text-muted-foreground">
              {row.itemVariant.item.category.industry ?? "—"}{" "}
              <span className="ml-1 text-xs">(set on the category in Main Database)</span>
            </p>
          </div>
